-- This project will analyze traffic accidents in the United States on highways in the year 2021. The data will
-- cover accident locations, accident times, lighting conditions, weather, collision types, intersection types,
-- land use, the presence of drunk drivers, etc.
...
-- 1. CONDITIONS THAT INCREASE THE RISK OF ACCIDENT
	...
	WITH dark_condition AS (SELECT 'Dark' AS light_condition,
							COUNT(CASE WHEN light_condition_name ILIKE '%- Lighted'
								  THEN consecutive_number END) AS lighted,
							COUNT(CASE WHEN light_condition_name ILIKE '%- Not Lighted'
								 
								  THEN consecutive_number END) AS not_lighted,
							COUNT(CASE WHEN light_condition_name ILIKE '%- Unknown Lighting'
								  THEN consecutive_number END) AS unknown_lighting
							FROM crash_2021
	...
	other_than_dark_condition AS (SELECT light_condition_name AS light_condition, 
								  ...
								  FROM crash_2021
								  WHERE light_condition_name NOT IN ('Dark - Lighted', 'Dark - Not Lighted',
